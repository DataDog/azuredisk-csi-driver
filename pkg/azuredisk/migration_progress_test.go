...
func TestHandlePVMigrationEvent(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			called := false
			testDriver := &testDriverCore{
				DriverCore:           *driver,
				updateProgressCalled: &called,
			}

			testDriver.handlePVMigrationEvent(tt.pv)
...
// testDriverCore extends DriverCore for testing
type testDriverCore struct {
	DriverCore
	updateProgressCalled *bool
}

func (t *testDriverCore) updateSinglePVMigrationProgress(pv *corev1.PersistentVolume) {
	if t.updateProgressCalled != nil {
		*t.updateProgressCalled = true
	}
	// Don't call parent method to avoid nil pointer issues in tests
}