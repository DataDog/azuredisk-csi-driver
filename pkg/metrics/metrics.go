...
var (
	...
	operationTotalByNodeLabel = metrics.NewCounterVec(
		&metrics.CounterOpts{
			Subsystem:      subSystem,
			Name:           "operations_total_by_node",
...
func init() {
	...
	legacyregistry.MustRegister(operationTotalByNodeLabel)
}
...
func (mc *CSIMetricContext) Observe(success bool) {
	...
	if mc.targetNode != "" {
		operationTotalByNodeLabel.WithLabelValues(mc.operation, successStr, mc.targetNode).Inc()
	}
